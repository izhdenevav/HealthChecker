...
model_points = np.array([
    ...
    (0.0, 300.0, -250.0),    # 7. Верх лба
    (-200.0, 100.0, -175.0), # 8. Левая скула
    (200.0, 100.0, -175.0)   # 9. Правая скула
], dtype=np.float64)
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    image_points = np.array([
                        [face_landmarks.landmark[1].x * w + x, face_landmarks.landmark[1].y * h + y],
                        [face_landmarks.landmark[199].x * w + x, face_landmarks.landmark[199].y * h + y],
                        [face_landmarks.landmark[33].x * w + x, face_landmarks.landmark[33].y * h + y],
                        [face_landmarks.landmark[263].x * w + x, face_landmarks.landmark[263].y * h + y],
                        [face_landmarks.landmark[61].x * w + x, face_landmarks.landmark[61].y * h + y],
                        [face_landmarks.landmark[291].x * w + x, face_landmarks.landmark[291].y * h + y],
                        [face_landmarks.landmark[10].x * w + x, face_landmarks.landmark[10].y * h + y],
                        [face_landmarks.landmark[234].x * w + x, face_landmarks.landmark[234].y * h + y],
                        [face_landmarks.landmark[454].x * w + x, face_landmarks.landmark[454].y * h + y]
                    ], dtype=np.float64)

                    # === Решаем PnP для определения поворота головы ===
                    _, rotation_vector, translation_vector = cv2.solvePnP(
                        model_points, image_points, camera_matrix, None, flags=cv2.SOLVEPNP_DLS
                    )
